...
class CreativeAgent:
    """Runs the creative content generator. Separate from ContentAgent so
    creative-specific tuning (angle selection, anime list, etc.) stays isolated."""
    ...
    async def run(self, raw_contents: List[RawContent], run_id: UUID) -> List[Post]:
        logger.info(
            "CreativeAgent: generating creative posts from %d raw items",
            len(raw_contents),
        )
        ...
        logger.info("CreativeAgent: saved %d posts with status=pending", len(posts))
        return posts
